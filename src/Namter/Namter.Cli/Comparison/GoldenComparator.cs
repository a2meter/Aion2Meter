...
public static class GoldenComparator
{
    public static ComparisonReport Compare(string actualRecordPath, ReadableFixture expected)
    {
        ...
        void Eq<T>(string field,T exp,T act) where T:notnull {if(EqualityComparer<T>.Default.Equals(exp,act))matches.Add(field);else diffs.Add(new(field,F(exp),F(act),$"actual.{field} vs readable fixture"));}
    }
