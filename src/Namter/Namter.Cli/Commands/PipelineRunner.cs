...
internal static class PipelineRunner
{
    ...
    {
        var allRecords=new List<EncounterRecord>();var allEvents=new List<CombatEvent>();var allDiagnostics=new List<NativeDiagnostic>();var allReasons=new List<IncompleteReasonRecord>();var hashes=new List<(string File,string Hash)>();ulong overlaps=0,duplicateBytes=0,unresolvedGaps=0;
        ...
        string inputRoot=CommonInputRoot(files);
        foreach(string file in files)
        {
            if(new FileInfo(file).Length>MaxReplayFileBytes)throw new InvalidDataException($"Replay file exceeds the {MaxReplayFileBytes}-byte bound: {file}");byte[] pcap=await File.ReadAllBytesAsync(file,cancellationToken);string hash=CommandSupport.Sha256(pcap);string relative=Path.GetRelativePath(inputRoot,Path.GetFullPath(file)).Replace('\\','/');hashes.Add((relative,hash));
            PipelineResult result=await RunOneAsync(NativeSourceKind.Pcap,pcap,snapshot,nativeSnapshot,$"pcap:{relative}:{hash}",speed,cancellationToken);
            if(allEvents.Count>MaxArtifactEvents-result.Events.Length)throw new InvalidDataException("Replay event-ledger bound exceeded.");allRecords.AddRange(result.Records);allEvents.AddRange(result.Events);allDiagnostics.AddRange(result.Diagnostics.Take(Math.Max(0,MaxDiagnostics-allDiagnostics.Count)));allReasons.AddRange(result.IncompleteReasons);overlaps=checked(overlaps+result.TcpOverlaps);duplicateBytes=checked(duplicateBytes+result.TcpDuplicateBytesRemoved);unresolvedGaps=checked(unresolvedGaps+result.TcpUnresolvedByteGaps);
        }
        WriteArtifacts(output,allEvents,allRecords,allDiagnostics,MergeReasons(allReasons),"pcap",speed,hashes,snapshot,overlaps,duplicateBytes,unresolvedGaps);await log.WriteLineAsync($"Replay complete: {files.Length} file(s), {allEvents.Count} event(s), {allRecords.Count} encounter(s).");return 0;
    }

    private static string CommonInputRoot(string[] files)
    {
        if(files.Length==0)throw new ArgumentException("At least one replay file is required.",nameof(files));
        string root=Path.GetDirectoryName(Path.GetFullPath(files[0]))??throw new InvalidDataException("Replay file has no parent directory.");
        foreach(string file in files.Skip(1))
        {
            string full=Path.GetFullPath(file);
            while(!IsBelow(root,full))root=Directory.GetParent(root)?.FullName??
                throw new InvalidDataException("Replay files must share one filesystem root.");
        }
        return root;
    }
    ...
    internal static async Task<T> SuperviseSourceAndConsumerAsync<T>(Task sourceTask,Task<T> consumeTask,ChannelWriter<CombatEvent> writer,CancellationTokenSource sourceCts,bool flushOnSourceCancel=false)
    {
        try
        {
            Task first=await Task.WhenAny(sourceTask,consumeTask);
            if(first==consumeTask)
            {
                sourceCts.Cancel();writer.TryComplete();
    ...
    {
        var events=new List<CombatEvent>();var records=new List<EncounterRecord>();var identities=new Dictionary<uint,ActorObservedEvent>();EncounterReducer reducer=NewReducer(snapshot,captureId,0);long lastMs=0;int index=0;
        await foreach(CombatEvent value in reader.ReadAllAsync(cancellationToken)){if(events.Count>=MaxArtifactEvents)throw new InvalidDataException("Event-ledger bound exceeded.");events.Add(value);if(value is ActorObservedEvent actor)identities[actor.ActorId]=actor;lastMs=checked((long)(value.Provenance.LastTimestampNs/1_000_000));EncounterUpdate update=reducer.Apply(value);if(update.FinalRecord is not null){records.Add(update.FinalRecord);reducer=NewReducer(snapshot,captureId,++index);foreach(ActorObservedEvent identity in identities.Values.OrderBy(x=>x.ActorId))reducer.Apply(identity);}}
    ...
    private static EncounterReducer NewReducer(GameDataSnapshot snapshot,string captureId,int index)=>new(snapshot,new EncounterReducerOptions(30_000,CommandSupport.StableGuid($"{captureId}\0{index}"),typeof(PipelineRunner).Assembly.GetName().Version?.ToString()??"0",1,"native",captureId,RequireCombatStart:true,PreserveBuffObservations:true,CarryInitialBuffState:index==0));
    ...
    private sealed record PipelineResult(ImmutableArray<CombatEvent> Events,ImmutableArray<EncounterRecord> Records,ImmutableArray<NativeDiagnostic> Diagnostics,ImmutableArray<IncompleteReasonRecord> IncompleteReasons,ulong TcpOverlaps,ulong TcpDuplicateBytesRemoved,ulong TcpUnresolvedByteGaps);
